...
fun fourth (xs) = List.nth(xs,3)

(* Write a function min3 of type int * int * int -> int that returns
the smallest of three integers *)

fun min3(x:int,y:int,z:int) = if x < y andalso x < z then x else if y < x andalso y < z then y else z;
...
fun thirds (x:string) = String.sub(x, 2)
