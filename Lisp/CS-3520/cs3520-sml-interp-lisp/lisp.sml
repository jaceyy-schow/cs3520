    ...
    let fun f isFile stream line =
            ...
            | SOME (line1, tokens) =>
                    ...
                    (case parseInput tokens of NONE =>
                        (* parse error *)
                        (TextIO.output (TextIO.stdErr, "Syntax error on input that started on line "^(Int.toString line)^"\n");
                    ...
                    | SOME (Expression exp, _) =>
                        ...
                        handle (RuntimeError msg) =>
                            (TextIO.output (TextIO.stdErr, msg^" (on input that started on line "^(Int.toString line)^")\n");
                             TextIO.flushOut TextIO.stdErr);
                        f isFile stream line1))
    in
